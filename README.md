...
전통 찬송가 100곡(빠른곡·미디움·느린곡 분류)은 `scripts/source-hymns.txt`에서
가져옵니다. 아래 스크립트가 제목이 일치하는 기존 곡에는 `isHymn` 플래그를 달고(코드·템포는
파일 값으로 갱신), 일치하는 곡이 없으면 새 항목으로 추가합니다(주제는 `THEME_BY_NO`로
분류). 둘러보기의 **분류** 탭에서 전체·찬송가·CCM 중 하나를 골라 좁힐 수 있습니다.

```bash
node scripts/hymns.mjs   # 찬송가 표시·추가 후 songs.json 갱신
```
...
- **둘러보기** — 분류 / 코드 / 주제 / 템포 축을 한 화면에서 전환, 가나다·최근 사용순 정렬
  (분류 탭에서 전체·찬송가·CCM 중 하나를 골라 좁힐 수 있습니다)
